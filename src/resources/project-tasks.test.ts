...
describe("registerProjectTasksResource", () => {
  ...
  it("should register project-tasks resource template", () => {
    ...
    expect(registeredResources.has("project-tasks")).toBe(true);
  });

  describe("resource handler", () => {
    it("should return tasks data on success", async () => {
      const mockTasks = [
        { oid: "task-1", id: 1, name: "Task 1" },
        { oid: "task-2", id: 2, name: "Task 2" },
      ];
      ...
      const resource = registeredResources.get("project-tasks");
      expect(resource).toBeDefined();
      if (!resource) throw new Error("Resource not found");

      const mockUri = new URL("quire://project/proj1/tasks");
      ...
      expect(JSON.parse(result.contents[0]?.text ?? "")).toEqual(mockTasks);
      expect(mockClient.listTasks).toHaveBeenCalledWith("proj1");
    });
    ...
    it("should throw error when project ID is missing", async () => {
      ...
      const mockUri = new URL("quire://project//tasks");
    ...
    it("should throw error on client failure", async () => {
      ...
      ).rejects.toThrow("Failed to fetch tasks: NOT_FOUND - Project not found");
    });
